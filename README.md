...
Currently, I plan on doing this through using an FPGA and an ESP32. The ESP32 will be able to handle the infrared light and receiver, as well as the expanded internet capabilties. The FPGA will be in charge of controlling the sd card and the ds cartidge protocol, althogugh, the ESP32 is capable of handeling all these tasks by itself, so in the end the DS nitro might only be powered by an ESP32. Using only an ESP32 will also allow the csr of the flash cart to be brought down.
